#!/bin/bash
# Deploy Google Chat MCP Server to GCP Cloud Run
# Run from: mcp-servers/google-chat-mcp/
#
# Prerequisites:
#   - gcloud CLI authenticated: gcloud auth login
#   - APIs enabled: Cloud Build, Cloud Run, Container Registry, Firestore
#   - Env vars set (see below)
#   - Google OAuth client has Chat API scopes enabled in GCP Console
#
# Usage:
...
REGION="us-central1"
SERVICE_NAME="google-chat-mcp"
IMAGE="gcr.io/$GCP_PROJECT/$SERVICE_NAME"
SERVICE_URL="https://google-chat-mcp-991974240717.us-central1.run.app"

# ── Validate env vars ─────────────────────────────────────────────────────────
for var in OAUTH_CLIENT_ID OAUTH_CLIENT_SECRET OAUTH_JWT_SECRET; do
  if [[ -z "${!var:-}" ]]; then
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-env-vars="OAUTH_CLIENT_ID=${OAUTH_CLIENT_ID},OAUTH_CLIENT_SECRET=${OAUTH_CLIENT_SECRET},OAUTH_JWT_SECRET=${OAUTH_JWT_SECRET},MCP_SERVER_URL=${SERVICE_URL}" \
  --port 3456 \
  --min-instances 0 \
  --max-instances 5 \
  --memory 256Mi \
