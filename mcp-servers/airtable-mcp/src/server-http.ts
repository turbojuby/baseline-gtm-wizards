...
export function startHttpServer(port: number): void {
  ...
  app.listen(port, () => {
    console.error(`[airtable-mcp] HTTP/SSE server listening on port ${port}`);
    console.error(`[airtable-mcp] SSE endpoint: http://localhost:${port}/sse`);
    console.error(`[airtable-mcp] Health check: http://localhost:${port}/health`);
  });
}
