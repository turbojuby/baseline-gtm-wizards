...
/**
 * Fathom MCP Server — Streamable HTTP transport entry point.
 *
 ...
 * so each user gets their own Fathom credentials from Firestore.
 *
...
app.get("/fathom/callback", handleFathomCallback);
...
app.post("/mcp", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error("[Fathom MCP] Error handling request:", error);
    if (!res.headersSent) {
      res.status(500).json({
        jsonrpc: "2.0",
...
app.get("/health", (_req: Request, res: Response) => {
  res.json({ status: "ok", server: "fathom-mcp", transport: "streamable-http" });
});
...
app.listen(PORT, () => {
  console.log(`[Fathom MCP] Streamable HTTP server listening on port ${PORT}`);
  console.log(`[Fathom MCP] MCP endpoint: http://localhost:${PORT}/mcp`);
  console.log(`[Fathom MCP] Health check: http://localhost:${PORT}/health`);
  if (process.env.OAUTH_JWT_SECRET) {
    console.log(`[Fathom MCP] OAuth 2.1 enabled (JWT signing active)`);
  }
});
